---
layout : single
title : "도커를 사용한 배포 자동화 시 도커 컨테이너 & 이미지 중첩으로 인한 리소스 부족으로 timeout 트러블 슈팅 (서버 다운)"
categories : til
...
일단 빠르게 문제를 해결하기 위해 모든 컨테이너를 삭제했습니다..
...
1번 방법은 검색 시 기존 컨테이너 중지 후 삭제하는 방법만 나오는 것으로 보아 지원하지 않는 매커니즘 같기도 합니다.
