...
spring.jpa.show-sql: JPA가 생성한 SQL문을 보여줄 지에 대한 여부를 알려주는 프로퍼티이다.

spring.jpa.generate-dll: 

spring.jpa.hibernate.dll-auto 옵션을 사용할 것인지를 결정하는 프로퍼티이다. **기본적으로 false**로 되어있기 때문에 JPA에 의한 데이터베이스 자동 초기화 기능을 사용하려면 true로 세팅해야 한다.
