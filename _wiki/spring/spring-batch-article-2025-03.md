...
이 데이터를 변환하고 **DB에 삽입하는 과정에서 가장 느린 배치(Batch) 로직부터 최적화하면, 과연 얼마나 빨라질까?** 하는 궁금증에서 시작된 과정을 공유하고자 합니다.
...
- **가정 및 검증할 포인트**
    ...
    - **데이터를 Sub-List로 나누어 비동기 병렬(parallel)처리를 적용한다.**
...
첫 번째 접근 방식은 **CSV 파일의 각 행을 개별적으로 파싱하고, JPA save()를 사용하여 INSERT**하는 것이었습니다.
...
- **스트리밍 방식의 핵심 개선점**
    - **메모리 사용량 감소**
    - **이전 방식(3차)에서는 모든 데이터를 List<Movie>에 저장**한 후, 이를 **배치 크기로 분할 후 병렬 처리**
        - **4차에서는 즉시 배치 크기만큼 쌓일 때마다 저장 & 초기화**
        - **불필요한 리스트 유지 방지 & GC 부담 감소**
- **배치 크기(BATCH_SIZE=500) 단위 비동기 실행**
    - CompletableFuture.runAsync()를 사용해 **병렬 실행 유지**
    - saveAllAndFlush()로 **즉시 플러시 → 데이터 지연 문제 방지**

이 접근 방식은 처리 시간을 **약 27초**로 단축시켰습니다.

### 5차: JDBC Template과 Bulk Insert

마지막으로, JPA 대신 **JDBC Template**을 사용하여 벌크 삽입(bulk insert)을 구현했습니다.
